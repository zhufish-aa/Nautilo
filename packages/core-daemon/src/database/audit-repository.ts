...
export class AuditRepository {
  ...
  append(record: AuditRecord): void {
    this.db.prepare("INSERT INTO audit_logs(id, action, resource_id, outcome, data, timestamp) VALUES(?, ?, ?, ?, ?, ?)")
      .run(record.id, record.action, record.resourceId ?? null, record.outcome, JSON.stringify(record), record.timestamp);
  }
