...
import { GitFork, FileDiff, MessagesSquare, Package, SquareTerminal } from "lucide-react";
...
function ChatArea({
  ...
  return (
    <>
              ...
              <section key={turn.key} className="turn-block pl-6" data-live={live}>
                {turn.header && (
                  <TimelineEventView
                    ...
                    onRevertCheckpoint={checkpoints.length > 0 ? openRevert : undefined}
                  />
                )}
                {turn.items.length > 0 && (
      ...
      <ArtifactsDrawer open={drawer === "artifacts"} onClose={onCloseDrawer} sessionId={sessionId} focusPath={diffFocusPath} />
